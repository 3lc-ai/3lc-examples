---
orphan: true
---

# 3 Training And Metrics

Integrate 3LC with popular machine learning frameworks to collect metrics, embeddings, and predictions during training. These tutorials demonstrate how to use 3LC with PyTorch, PyTorch Lightning, Hugging Face Transformers, Detectron2, Ultralytics YOLO, and SuperGradients.

Learn to track model performance, collect embeddings for analysis, fine-tune models on custom datasets, and leverage 3LC's capabilities for improved model development and debugging across classification, detection, segmentation, and pose estimation tasks.

```{toctree}
:hidden:
:glob:
:maxdepth: 2

./*
./*/index
```
