...
class ImageProcessingResult:
    ...
    new_size: Optional[Tuple[int, int]]  # (width, height) or None if not changed
    original_file_size: int  # bytes
    new_file_size: Optional[int]  # bytes or None if not changed
    processed: bool
...
class NormalizationSettings:
    ...
    max_dimension: int = 1000  # Maximum width or height (good for notebook titles + galleries)
...
class ImageNormalizer:
    ...
    def __init__(self, settings: Optional[NormalizationSettings] = None):
    ...
    def print_summary(self, results: List[ImageProcessingResult], dry_run: bool = False) -> None:
        ...
        if not dry_run:
            total_new_size = sum(
                r.new_file_size if r.new_file_size is not None else r.original_file_size 
                for r in results
            )
        ...
        if processed_count > 0:
            ...
            for result in results:
                if result.processed:
                    ...
                    print(f"  {result.path.name}: {old_size} → {new_size}, "
                          f"{old_file_size:.1f}KB → {new_file_size:.1f}KB")
