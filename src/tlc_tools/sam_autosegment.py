...
def bbs_to_segments(
    ...
    # BoundingBox type to allow arbitrary bounding box types as input
    bb_type = tlc.BoundingBox.from_schema(input_table.rows_schema.values[bb_column].values["bb_list"])

    # The value map from the bb column will be used in the output table's segmentation column
    value_map = input_table.get_value_map(f"{bb_column}.bb_list.label")

    if not value_map:
        raise ValueError(f"Could not find value map for column {bb_column}.bb_list.label")
    ...
    table_writer = tlc.TableWriter(
        ...
        column_schemas={
            image_column: tlc.ImageUrlStringValue(),
    ...
    for row in tqdm.tqdm(input_table, desc="Processing rows", total=len(input_table)):
        ...
        for bb in row[bb_column]["bb_list"]:
            box_arr = np.array(
